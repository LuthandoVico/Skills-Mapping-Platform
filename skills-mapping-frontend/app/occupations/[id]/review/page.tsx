...
type AIDecision = "pending" | "accepted" | "rejected";
type ReviewTab = "overview" | "skills" | "education" | "labour-market";
...
export default function ExpertReviewPage() {
  ...
  const [aiStates, setAiStates] = useState<Record<string, { decision: AIDecision; comment: string }>>(
    Object.fromEntries(occupationDetail.aiRecommendations.map((r) => [r.id, { decision: "pending" as AIDecision, comment: "" }]))
  );
    ...
    return (
      <>
        ...
        <main className="flex-grow flex flex-col items-center justify-center min-h-[80vh] px-8">
          ...
          <h2 className="text-[24px] font-extrabold text-black">Review Submitted</h2>
          <p className="text-[14px] text-[#6b7280] mt-2 text-center max-w-[480px] leading-[22px]">
            Your expert review for <span className="font-bold text-black">{occupationDetail.title}</span> has been submitted for consensus.
          </p>
  ...
  return (
    <>
      ...
      {/* ════ REVIEW-MODE HEADER (replaces OccupationHeader) ════ */}
      <div className="bg-white border-b border-[#d4af37]/30 pt-24 pb-0">
        <div className="max-w-[1400px] mx-auto px-8">
            ...
            <div className="max-w-[700px]">
              ...
              <div className="flex items-center gap-3 mt-3">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-[#d4af37]/10 rounded-lg border border-[#d4af37]/20">
                  ...
                  <span className="text-[11px] font-bold text-[#d4af37] uppercase tracking-wider">Expert Annotation Mode</span>
                </div>
          ...
          {/* Tab Navigation — mirrors original but within review mode */}
      ...
      <main className="flex-grow max-w-[1400px] mx-auto px-8 py-10 w-full pb-24">
        <div className={`grid gap-8 ${aiPanelOpen ? "grid-cols-1 lg:grid-cols-[1fr_320px]" : "grid-cols-1"}`}>

          {/* ── Left: Tab Content (mirrors original layouts) ── */}
          <div>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-20">
                <div className="bg-white border border-[#e5e7eb] rounded-2xl shadow-sm overflow-hidden">
                  <div className="p-4 border-b border-[#e5e7eb] bg-gradient-to-r from-indigo-50 to-blue-50 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[13px] font-bold text-[#1d3557]">AI Suggestions</span>
                    </div>
                    <button onClick={() => setAiPanelOpen(false)} className="text-gray-400 hover:text-gray-600 transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                  </div>
                  <div className="p-3 flex flex-col gap-2.5 max-h-[calc(100vh-220px)] overflow-y-auto">
                    {occupationDetail.aiRecommendations.map((rec) => {
                      const st = aiStates[rec.id];
                      const colors: Record<string, string> = { missing_skill: "bg-emerald-50 text-emerald-700 border-emerald-200", update_education: "bg-blue-50 text-blue-700 border-blue-200", outdated_skill: "bg-orange-50 text-orange-700 border-orange-200", update_market: "bg-purple-50 text-purple-700 border-purple-200", add_task: "bg-teal-50 text-teal-700 border-teal-200" };
                      const labels: Record<string, string> = { missing_skill: "Missing Skill", update_education: "Update Education", outdated_skill: "Outdated Skill", update_market: "Update Market", add_task: "Add Task" };
                      return (
                        <div key={rec.id} className={`p-3 rounded-xl border transition-all ${st.decision === "accepted" ? "bg-green-50 border-green-200" : st.decision === "rejected" ? "bg-red-50/50 border-red-200 opacity-60" : "bg-white border-[#e5e7eb]"}`}>
                          <div className="flex items-start justify-between gap-2 mb-1.5">
                            <span className={`px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider rounded border ${colors[rec.type] ?? ""}`}>{labels[rec.type] ?? rec.type}</span>
                            <span className="text-[9px] font-bold text-[#6b7280]">{Math.round(rec.confidence * 100)}%</span>
                          </div>
                          <h4 className="text-[12px] font-bold text-black leading-[16px]">{rec.title}</h4>
                          <p className="text-[10px] text-[#6b7280] mt-0.5 leading-[14px]">{rec.reason}</p>
                          {st.decision === "pending" ? (
                            <div className="flex gap-1.5 mt-2">
                              <button onClick={() => setAiStates((p) => ({ ...p, [rec.id]: { ...p[rec.id], decision: "accepted" } }))}
                                className="flex-1 px-2 py-1 text-[9px] font-bold text-green-700 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors">Accept</button>
                              <button onClick={() => setAiStates((p) => ({ ...p, [rec.id]: { ...p[rec.id], decision: "rejected" } }))}
                                className="flex-1 px-2 py-1 text-[9px] font-bold text-red-700 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100 transition-colors">Reject</button>
                            </div>
                          ) : (
                            <div className="mt-1.5 flex items-center gap-1.5">
                              <span className={`text-[9px] font-bold uppercase ${st.decision === "accepted" ? "text-green-600" : "text-red-500"}`}>
                                {st.decision === "accepted" ? "✓ Accepted" : "✕ Rejected"}
                              </span>
                              <button onClick={() => setAiStates((p) => ({ ...p, [rec.id]: { ...p[rec.id], decision: "pending" } }))}
                                className="text-[9px] text-[#6b7280] hover:text-[#1d3557] font-medium">undo</button>
                            </div>
                          )}
                          <input type="text" value={st.comment} onChange={(e) => setAiStates((p) => ({ ...p, [rec.id]: { ...p[rec.id], comment: e.target.value } }))}
                            placeholder="Rationale..." className="w-full mt-1.5 border border-[#e5e7eb] rounded-lg px-2 py-1 text-[10px] text-gray-700 focus:outline-none focus:ring-1 focus:ring-[#d4af37]/30 bg-white/80" />
                        </div>
                      );
                    })}
                  </div>
        ...
        {/* Toggle AI panel if closed */}
        {!aiPanelOpen && (
          <button onClick={() => setAiPanelOpen(true)}
            className="fixed right-6 top-1/2 -translate-y-1/2 z-40 bg-indigo-600 text-white p-3 rounded-xl shadow-lg hover:bg-indigo-700 transition-colors">
